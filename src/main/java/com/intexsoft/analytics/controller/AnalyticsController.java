...
public class AnalyticsController {
    ...
    @Operation(description = "Find salary forks for different title for specific Department")
    @PreAuthorize("@serviceAccess.canAccessToDepartmentData(authentication, #id)")
    public Mono<SalaryAnalyticsDto> findDepartmentSalaryForks(@PathVariable UUID id) {
        log.debug("Requested Find department salary border for department:{}", id);
        return analyticsService.findDepartmentSalaryForks(id);
    }
